...
interface Column {
  ...
  sortable: boolean;
  filterable: boolean;
  render: (expectedIncome: ExpectedIncome) => React.ReactNode;
}
...
export default function ExpectedIncomeTable({
  ...
  const getFilteredExpectedIncomes = () => {
    let filtered = expectedIncomes;

    Object.keys(filterState).forEach((columnKey) => {
      const filterValues = filterState[columnKey];
      if (filterValues.length === 0) return;

      filtered = filtered.filter((expectedIncome) => {
        let value: string;
        
        switch (columnKey) {
          case 'source':
            value = expectedIncome.source_name || '';
            break;
          case 'description':
            value = expectedIncome.description || '';
            break;
          case 'type':
            value = expectedIncome.is_manual ? 'ידני' : 'אוטומטי';
            break;
          case 'categories':
            const categoryNames = expectedIncome.categories?.map(c => c.name) || [];
  ...
  const getSortedExpectedIncomes = () => {
    const filtered = getFilteredExpectedIncomes();
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      switch (sortState.column) {
        ...
        case 'amount':
          ...
          break;
        case 'type':
          aValue = a.is_manual ? 1 : 0;
          bValue = b.is_manual ? 1 : 0;
          break;
        case 'created_at':
  ...
  const getUniqueValues = (columnKey: string): string[] => {
    ...
    expectedIncomes.forEach((expectedIncome) => {
      let value: string;
      ...
      switch (columnKey) {
        ...
        case 'type':
          ...
          break;
        case 'categories':
          expectedIncome.categories?.forEach(cat => values.add(cat.name));
          return;
        default:
          return;
      }
  ...
  const columns: Column[] = [
    {
      ...
      sortable: true,
      filterable: true,
      render: (expectedIncome: ExpectedIncome) => (
        <span>{expectedIncome.source_name || '-'}</span>
      ),
    },
    {
      key: 'amount',
      ...
      filterable: false,
      render: (expectedIncome: ExpectedIncome) => (
        <span style={styles.amount}>{formatCurrency(expectedIncome.amount)}</span>
      ),
    },
    {
      key: 'description',
      ...
      filterable: true,
      render: (expectedIncome: ExpectedIncome) => (
        <span style={styles.description}>{expectedIncome.description || '-'}</span>
      ),
    },
    {
      key: 'categories',
      ...
      render: (expectedIncome: ExpectedIncome) => (
        <div style={styles.categoriesCell}>
          {expectedIncome.categories && expectedIncome.categories.length > 0 ? (
            expectedIncome.categories.map((category) => (
              <span
                key={category.id}
                style={{
    ...
    },
    {
      ...
      render: (expectedIncome: ExpectedIncome) => (
        <span
          style={{
            ...styles.typeBadge,
            ...(expectedIncome.is_manual ? styles.manualBadge : styles.autoBadge),
          }}
        >
          {expectedIncome.is_manual ? 'ידני' : 'אוטומטי'}
        </span>
      ),
    },
  ];
    ...
    columns.push({
      ...
      render: (expectedIncome: ExpectedIncome) => (
        <div style={styles.actionsCell}>
          {onEdit && (
            <button
              onClick={() => onEdit(expectedIncome.id)}
              style={styles.actionBtn}
              className="action-btn edit-btn"
              title="ערוך"
              aria-label="ערוך הכנסה צפויה"
            >
              ✏️
            </button>
          )}
          {onDelete && (
            <button
              onClick={() => onDelete(expectedIncome.id)}
              style={{ ...styles.actionBtn, ...styles.deleteBtn }}
              className="action-btn delete-btn"
              title="מחק"
              aria-label="מחק הכנסה צפויה"
            >
              🗑️
            </button>
          )}
  ...
  if (expectedIncomes.length === 0) {
    return (
      <div style={styles.emptyState}>
        <p>אין הכנסות צפויות להצגה</p>
      </div>
    );
  }

  const sortedExpectedIncomes = getSortedExpectedIncomes();
  ...
  return (
    <div style={styles.tableContainer} ref={tableRef}>
      ...
      <table style={styles.table} className={tableClassName}>
        ...
        <tbody>
          {sortedExpectedIncomes.map((expectedIncome) => (
            <tr key={expectedIncome.id} style={styles.row} className="table-row">
              {columns.map((column) => (
                <td key={column.key} style={styles.cell}>
                  {column.render(expectedIncome)}
                </td>
              ))}
            </tr>
          ))}
...
`;
if (!document.head.querySelector('style[data-expected-income-table]')) {
  tableHoverStyle.setAttribute('data-expected-income-table', 'true');
